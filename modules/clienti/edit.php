...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_valid_csrf();
    $fields = ['nome', 'cognome', 'cf_piva', 'email', 'telefono', 'indirizzo', 'note'];
    ...
    if (!$errors) {
        $update = $pdo->prepare('UPDATE clienti SET nome = :nome, cognome = :cognome, cf_piva = :cf_piva, email = :email, telefono = :telefono, indirizzo = :indirizzo, note = :note WHERE id = :id');
        $update->execute([
            ':nome' => $data['nome'],
        ...
        $logStmt->execute([
            ...
            ':dettagli' => sprintf('%s %s (#%d)', $data['cognome'], $data['nome'], $id),
        ]);
        ...
        if ($integrationWarning) {
            add_flash('warning', 'Cliente aggiornato ma sincronizzazione ERP non riuscita. Controlla integrations.log.');
        }
