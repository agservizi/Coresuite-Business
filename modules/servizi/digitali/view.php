...
$pageTitle = 'Dettaglio servizio digitale';
...
$stmt = $pdo->prepare('SELECT sd.*, c.nome, c.cognome, c.email FROM servizi_digitali sd LEFT JOIN clienti c ON sd.cliente_id = c.id WHERE sd.id = :id');
$stmt->execute([':id' => $id]);
$record = $stmt->fetch();
if (!$record) {
...
<div class="flex-grow-1 d-flex flex-column min-vh-100">
    ...
    <main class="content-wrapper">
        <div class="page-toolbar mb-4">
            <a class="btn btn-outline-warning" href="index.php"><i class="fa-solid fa-arrow-left"></i> Tutte le pratiche</a>
            ...
            <div class="col-md-6">
                <div class="card ag-card h-100">
                    <div class="card-header bg-transparent border-0">
                        <h5 class="card-title mb-0">Dettagli pratica</h5>
                    </div>
                    <div class="card-body">
                        <dl class="row mb-0">
        ...
        <div class="card ag-card mt-4">
            <div class="card-header bg-transparent border-0">
                <h5 class="card-title mb-0">Documentazione</h5>
            </div>
            <div class="card-body">
                <?php if ($record['documento_path']): ?>
                    <a class="btn btn-warning text-dark" href="../../../<?php echo sanitize_output($record['documento_path']); ?>" target="_blank"><i class="fa-solid fa-download me-2"></i>Scarica documento</a>
                <?php else: ?>
                    <p class="text-muted mb-0">Nessun documento allegato.</p>
                <?php endif; ?>
            </div>
        </div>
    </main>
