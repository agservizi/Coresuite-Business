...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (is_array($experiencesRaw)) {
        foreach ($experiencesRaw as $row) {
            ...
            if (!$validateDate($startDate) || !$validateDate($endDate)) {
                $errors[] = 'Le date inserite nelle esperienze non sono valide.';
                continue;
            }
            if ($role === '' && $employer === '' && $startDate === '' && $endDate === '' && trim((string) ($row['description'] ?? '')) === '') {
                continue;
            }
    ...
    if (is_array($educationRaw)) {
        foreach ($educationRaw as $row) {
            ...
            $endDate = trim((string) ($row['end_date'] ?? ''));
            if (!$validateDate($startDate) || !$validateDate($endDate)) {
                $errors[] = 'Le date inserite nella formazione non sono valide.';
                continue;
            }
            if ($title === '' && $institution === '' && $startDate === '' && $endDate === '' && trim((string) ($row['description'] ?? '')) === '') {
                continue;
            }
            $sectionData['education'][] = [
                ...
                'start_date' => $startDate,
                'end_date' => $endDate,
                'qualification_level' => trim((string) ($row['qualification_level'] ?? '')),
                'description' => trim((string) ($row['description'] ?? '')),
            ];
        }
    }
    ...
    if (is_array($languagesRaw)) {
        foreach ($languagesRaw as $row) {
            ...
            $sectionData['languages'][] = [
                'language' => $language,
                'overall_level' => in_array(($row['overall_level'] ?? ''), $languageLevels, true) ? (string) $row['overall_level'] : '',
