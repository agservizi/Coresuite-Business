                        ...
                        <tbody>
                            <?php foreach ($records as $cv): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge ag-badge text-uppercase <?php echo $cv['status'] === 'Pubblicato' ? 'bg-success' : ($cv['status'] === 'Archiviato' ? 'bg-secondary' : 'bg-warning text-dark'); ?>">
                                    ...
                                    <td>
                                        <?php if ($cv['generated_file']): ?>
                                            <a class="btn btn-sm btn-outline-warning" href="../../../<?php echo sanitize_output($cv['generated_file']); ?>" target="_blank" rel="noopener">PDF</a>
                                        <?php else: ?>
                                    ...
                                    <td class="text-end">
                                        <div class="btn-group">
                                            <a class="btn btn-sm btn-outline-warning" href="view.php?id=<?php echo (int) $cv['id']; ?>" title="Dettagli" aria-label="Dettagli"><i class="fa-solid fa-eye"></i></a>
                                            <a class="btn btn-sm btn-outline-warning" href="wizard.php?id=<?php echo (int) $cv['id']; ?>" title="Modifica" aria-label="Modifica"><i class="fa-solid fa-pen"></i></a>
                                            <form method="post" action="publish.php" class="d-inline">
                                                <input type="hidden" name="_token" value="<?php echo sanitize_output($csrfToken); ?>">
                                                <input type="hidden" name="id" value="<?php echo (int) $cv['id']; ?>">
                                                <button class="btn btn-sm btn-outline-warning" type="submit" title="Genera PDF" aria-label="Genera PDF"><i class="fa-solid fa-file-pdf"></i></button>
                                            </form>
                                            <form method="post" action="delete.php" class="d-inline" onsubmit="return confirm('Confermi l\'eliminazione del curriculum? Questa operazione è irreversibile.');">
                                                <input type="hidden" name="_token" value="<?php echo sanitize_output($csrfToken); ?>">
                                                <input type="hidden" name="id" value="<?php echo (int) $cv['id']; ?>">
                                                <button class="btn btn-sm btn-outline-warning" type="submit" title="Elimina" aria-label="Elimina"><i class="fa-solid fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
